...
void getScansFromBag(const string& bag_name,
...
void applyTruncate(const float new_max_range, vector<vector<float>>* all_scans) {
  for (size_t i = 0; i < all_scans->size(); ++i) {
    vector<float> single_scan = all_scans[0][i];
    for (size_t j = 0; j < single_scan.size(); ++j) {
      if (single_scan[j] >= new_max_range) { // truncate ==> set max range+ to 0
        single_scan[j] = 0.0;
      }
    }
  }
...
void applyHumans(const int FOV, vector<vector<float>>* all_scans) {
  ...
  const int num_obs = all_scans[0][0].size();
  ...
  for (size_t i = 0; i < all_scans->size(); ++i) {
    ...
    const float r_ub = std::max(min_human_dist, single_scan[r_ub_idx]);
    float r = 1.0;
    if (r_ub > r) {
    ...
    const int start_idx1 = (phi - (theta_T / 2.0)) * (num_obs / FOV_rad);
    const int start_idx2 = (phi - (theta_s / 2.0)) * (num_obs / FOV_rad);
    ...
    const float L2y = r * cos(phi + (theta_d/2.0) + (theta_s/2.0));
...
void applyCorruption(const float FOV,
  ...
  if (truncate) {
    float new_max_range = 10.0;
    applyTruncate(new_max_range, all_scans);
  }
}

void convertScansToImagesPadAndFixedBounds(string bag_name, 
                                           const vector<vector<float>>& all_scans,
                                           const int FOV) {
  std::stringstream ss;
  ss.str(bag_name);
  string item;
...
int main(int argc, char* argv[]) {
  if (argc != 4) {
    printf("Usage: ./exec <path to bag file> <laser field of view in degrees> <max laser range in meters>");
    fflush(stdout);
    exit(1);
  }
  ...
  vector<vector<float>> all_scans;
  getScansFromBag(bag_name, laser_max_range, &all_scans);
  srand (static_cast <unsigned> (time(0)));
  applyCorruption(FOV, truncate, seasoning, humans, &all_scans);
  //convertScansToImages(bag_name, all_scans);
  convertScansToImagesPadAndFixedBounds(bag_name, all_scans, FOV);
  return 0;
}
